...
extrinsicTranslationVector = [0; 0; cameraDepthPositionMM - eyeRadius - pupilPlaneDepth];
...
renderedFramesDirectory = fullfile(exportsDirectory,'renderedFrames');
...
% Blender to render the scene. The assigned eyeRotationDepth is equal to
% the radius specified by the Blender model, plus the distance between the
% corneal surface and the pupil plane that the pupilProjection_fwd model
% assumes.
runVideoPipeline( pathParams, ...
    'verbosity', 'full', 'useParallel',false, 'catchErrors', false,...
    'glintGammaCorrection', 10, ...
    'pupilFrameMask', [60 60], 'maskBox', [1 1], 'pupilGammaCorrection',0.5, 'pupilRange', [20 80], ...
    'overwriteControlFile',true, 'glintPatchRadius', 15,'cutErrorThreshold',0.5, ...
    'candidateThetas',0:pi/16:2*pi,...
    'makeFitVideoByNumber',[3 6],...
    'skipStageByNumber',[1:1:1],...
    ...
    'eyeRotationDepth',eyeRadius+3.7,...
    'eyeRotationDepthLB',eyeRadius+3.7,...
    'eyeRotationDepthUB',eyeRadius+3.7,...
    'ellipseArrayList',1:1:nFrames ...
    );
